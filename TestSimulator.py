...
class TestSimulator(unittest.TestCase):
	...
	def testAddVehicle(self):
		sim = Simulator()
		pose = [0,0,0]
		gv1 = GroundVehicle(pose,1,1)
		gv2 = GroundVehicle(pose,0,1)
	...
	def testRemoveVehicle(self):
		...
		sim.removeGroundVehicle(gv1)

		self.assertEqual(gv2,sim._gvList[0])
		self.assertEqual(1, len(sim._gvList))

		sim.removeGroundVehicle(gv2)

		self.assertEqual([], sim._gvList)
	...
	def testinList(self):
		...
		self.assertEqual(True, sim.inList(gv1))
		self.assertEqual(True, sim.inList(gv2))

		gv3 = GroundVehicle(pose,4,0)

		self.assertEqual(False, sim.inList(gv3))
